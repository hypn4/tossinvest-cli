...
| `public` | `GET` | `wts-info-api.tossinvest.com` | `/api/v3/stock-prices/{code}/quotes` | orderbook (호가창) | `.result` with `offerPrices`/`offerVolumes`/`bidPrices`/`bidVolumes` (English keys), `singlePrice` flag, plus offer/bid totals | `quotes book <sym> (since v0.5.x)` | KR returns 10 levels; US returns top-of-book (1 level). v1/v2 variants use Korean-flavored keys; CLI uses v3 only. |
| `public` | `GET` | `wts-info-api.tossinvest.com` | `/api/v2/stock-prices/{code}/ticks?count=...` | recent trade ticks (체결 틱) | `.result[]` newest-first with `time` (HH:MM:SS), `code`, `price`, `priceKrw`, `base`, `volume`, `tradeType`, `cumulativeVolume` | `quotes ticks <sym> [--follow] (since v0.5.x)` | Dedup key = `cumulativeVolume` (monotonic per session); `time` lacks date. `--follow` streams oldest-first NDJSON. |
